...
for ii, line in enumerate(input.splitlines()):
    splitter_locations = {
        splitter_match.start(0) for splitter_match in re.finditer(r"\^", line)
    }
    ...
    if splitter_locations:
        for tachyon_location in tachyon_locations:
            if tachyon_location in splitter_locations:
                new_tachyon_locations.update(
                    [tachyon_location - 1, tachyon_location + 1]
                )
                total_splits += 1
            else:
    ...
    if splitter_locations:
        for tachyon_location in tachyon_locations:
            if tachyon_location in splitter_locations:
                ...
                total_routes[tachyon_location - 1] = (
                    total_routes[tachyon_location - 1] + total_routes[tachyon_location]
                )
                total_routes[tachyon_location + 1] = (
                    total_routes[tachyon_location + 1] + total_routes[tachyon_location]
                )
